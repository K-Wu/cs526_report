...
\begin{itemize}
    ...
    \item SIMD operation leverages data parallelism to boost performance.
...
\begin{frame}{Vectorizers in LLVM}
  \begin{block}{LLVM vectorizers are IR passes.}
  \begin{itemize}
  ...
  \begin{block}{}
    \begin{itemize}
      ...
      \item SLP immediately follows Loop Vectorizer in the optimization pipeline.
    \end{itemize}
  \end{block}
  %\pause
...
\begin{block}{We implement the stem part of the tree building and vectorization}
  \begin{itemize}
  \item Reduction and loop-awareness are not supported.
  \item Uses many helpers from LLVM 3.5.0, e.g., block scheduling, tree cost calculation, consecutive access, etc.
  \end{itemize}
...
\hypertarget{pruning}{%
\section{TSLP: Prune the Tree for More Profit}\label{pruning}}
...
\hypertarget{summary}{%
\section{Summary}\label{reference}}

\begin{frame}{Summary}
\begin{block}{}
\begin{itemize}
    \item Reproduced the stem of bottom-up SLP vectorizer, based on LLVM 3.5.0.
    \item Implemented the TSLP tree pruning to find the most profitable cut.
    \item Achieved up to 18.7\% speed up, and at least comparable performance, in several scientific workload, compared with LLVM 8.0.1 SLP implementation.
\end{itemize}
\end{block}
\end{frame}
